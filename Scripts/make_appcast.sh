...
ZIP=${1:?
"Usage: $0 TokenStats-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/steipete/TokenStats/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^TokenStats-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/steipete/TokenStats/releases/download/v${VERSION}/"}
...
WORK_DIR=$(mktemp -d /tmp/tokenstats-appcast.XXXXXX)
