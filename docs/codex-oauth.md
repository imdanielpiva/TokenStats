...
> Replicate Codex's direct OAuth token usage in TokenStats instead of calling the CLI.

## Background

Currently, TokenStats fetches Codex usage by:
...
- Command: `TokenStatsCLI usage --provider codex --source oauth --json --pretty`
...
|------|----------|---------|
| `CodexOAuthCredentials.swift` | `Sources/TokenStatsCore/Providers/Codex/CodexOAuth/` | Token storage model + loader |
| `CodexOAuthUsageFetcher.swift` | `Sources/TokenStatsCore/Providers/Codex/CodexOAuth/` | API client for usage endpoint |
| `CodexTokenRefresher.swift` | `Sources/TokenStatsCore/Providers/Codex/CodexOAuth/` | Token refresh logic |
...
public enum CodexOAuthUsageFetcher {
    ...
    public static func fetchUsage(
        ...
        request.setValue("TokenStats", forHTTPHeaderField: "User-Agent")
...
2. Run TokenStats with debug logging enabled
